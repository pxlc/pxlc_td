...
class TextOverlay(object):
    ...
    def __init__(self, font_size, fill_color_str, stroke_color_str='', stroke_pixel_width=1, ttf_filepath=''):
        ...
        self.font = ImageFont.truetype(self.ttf_filepath, size=font_size)

    def add_text_item(self, text_message, x, y):

        self.text_item_list.append({'text': text_message, 'pos': (x, y)})

    def get_text_size(self, text_message):
        return self.font.getsize(text_message)   # return the size of the text in tuple: (width, height)

    def _draw_text_message(self, draw_obj, x, y, text):
        ...
        if self.stroke_color_str is not None:
            ...
            for st_y in range(0-stroke_thickness, stroke_thickness+1):
                for st_x in range(0-stroke_thickness, stroke_thickness+1):
                    draw_obj.text((x+st_x, y+st_y), text, font=self.font, fill=self.stroke_color_str)

        draw_obj.text((x, y), text, fill=self.fill_color_str, font=self.font)
    ...
    def draw_items(self, pil_img_to_draw_on):
        ...
        for t_item in self.text_item_list:
            ...
            (x, y) = t_item.get('pos')
            self._draw_text_message(draw_obj, x, y, text)
